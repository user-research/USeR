...
class Small(BaseMetric):
    ...
    def _train_small(self):
        ...
        topic_model = BERTopic(
            ...
            # decrease the number of min topics 10 -> 3 detection to work with fewer data sets < ~1000
            min_topic_size=3,
        ...
        try:
            show_tree_topics_file = config['project']['show_tree_topics_file'].format(project=self.project)
            if not os.path.exists(show_tree_topics_file):
                html = self.topic_model.get_topic_tree(hierarchical_topics)
                func = open(show_tree_topics_file, "w")
                func.write(html)
                func.close()
        except Exception as e:
            logging.error(e)
    ...
    def small(self):
        ...
        m = hashlib.sha256((self.usid if not self.usid == None else '' + self.user_story).encode('UTF-8'))
        ...
        try:
            ...
            pd.DataFrame([entry]).to_csv(probs_cache_file, index=False, header=None, mode='a')

            # Run Generalized ESD Test for Outliers to detect outlier probabilities that are topic related
            num_topics = ESD_Test(probs, 0.05, 10)
            
            # In case of only one topic was found by BERTopic, we assume the story contains just this single topic
            if (max_topics - min_topics) == 0:
                self._small = 1.0
            # If we couldn't find outliers, we assume two cases:
            elif num_topics == 0:
                # The story contains/opens a new topic which not releates to existing topics, shown by low probabilities to all topics
                self._small = 1.0
                # The story is related to almost all topics, shown by high probabilities to all topics
            ...
            if config.getboolean('app', 'cache'):
                pd.DataFrame([[story_hash, self._small]]).to_csv(cache_file, index=False, header=None, mode='a')
        except Exception as e:
            logging.error(e)
...
def _calculate_critical_value(size, alpha):
    """
...
def _check_G_values(Gs, Gc, inp, max_index):
    """
